...
async function runMutation(callback: () => Promise<void>) {
  ...
  try {
    ...
    useAppStore.setState({ loading: false, error: message });
    throw error;
  }
}
...
export const useAppStore = create<AppState>()((set, get) => ({
  ...
  loadAttendanceByDate: async (date, force = false) => {
    ...
    // Guard definitivo usando Set para evitar bucle incluso con 0 registros
  ...
  setAttendance: async (record) => {
    ...
    try {
      ...
      // Marcar como cargada para evitar refetch redundante
      set((state) => {
        const newDates = new Set(state.fetchedDates);
  ...
  savePayroll: async (payroll) => {
    ...
    try {
      ...
      // 4. Optimized Local Update - Add the new payroll to the list manually
  ...
  toggleISR: () => {
    const current = useAppStore.getState().settings.isrHabilitado;
    const next = !current;
  ...
  addDiaEspecial: (fecha: string, multiplicador: number) => {
    const current = useAppStore.getState().settings.diasEspeciales;
